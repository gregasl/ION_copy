...
public class OrderCreator implements MkvFunctionCallListener, MkvPlatformListener, MkvRecordListener {
        ...
        try {
            ...
            if (orderCreator != null) {
                ...
                Thread monitorThread = new Thread(() -> {
                    try {
                        for (int i = 0; i < 15; i++) { // 15 seconds monitoring
                            Thread.sleep(1000);
                            logOrderStatus();
                        }
    ...
    public static OrderCreator createOrder() {
        ...
        // 尝试不同的instrument ID格式
        String[] instrumentVariants = {
            "912797QB6",                    // 原始CUSIP
            "912797QB6_C_Fixed",            // FENICS格式
            "912797QB6_C_Par_0",            // BTEC格式
            "912797QB6_C",                  // 简化格式
            "912797QK6",                    // 备用CUSIP
            "912797QK6_C_Fixed",            // 备用FENICS格式
            "912797QK6_C_Par_0"             // 备用BTEC格式
        };
        ...
        for (String marketSource : POSSIBLE_MARKET_SOURCES) {
            ...
            try {
                ...
                if (fn != null) {
                    ...
                    // 尝试不同的instrument ID
                    for (String instrumentId : instrumentVariants) {
                        LOGGER.info("Trying instrument: {}", instrumentId);
                        OrderCreator result = createOrderWithFunction(fn, marketSource, instrumentId);
                        if (result != null) {
                            // 检查是否成功创建了订单（不是-105错误）
                            OrderDetails details = orderTracking.get(String.valueOf(reqId));
                            if (details != null && !details.status.equals("INSTRUMENT_NOT_FOUND")) {
                                return result;
                            }
                            // 如果是-105错误，继续尝试下一个instrument
                            LOGGER.info("Instrument {} failed, trying next...", instrumentId);
                        }
                    }
                }
            } catch (Exception e) {
        ...
        LOGGER.error("No order functions available or all instruments failed!");
        ...
        LOGGER.error("4. The instrument IDs are valid for this market");
     ...
     */
    private static OrderCreator createOrderWithFunction(MkvFunction fn, String marketSource, String instrumentId) {
        try {
            // Create FreeText
            ...
            LOGGER.info("Calling order function with instrument: {}", instrumentId);
            ...
            LOGGER.error("Error creating order with instrument {}: {}", instrumentId, e.getMessage());
            return null;
        }
    }
    ...
    public void onResult(MkvFunctionCallEvent event, MkvSupply supply) {
        try {
            ...
            // 检查是否是-105错误
            if (resultString.contains("-105") || resultString.contains("INSTRUMENT NOT FOUND")) {
                LOGGER.info("║ ❌ INSTRUMENT NOT FOUND (-105)        ║");
                LOGGER.info("║ 来源: onResult回调                    ║");
                LOGGER.info("║ 完整响应: {}", resultString);
                LOGGER.info("║ 将尝试下一个instrument ID...          ║");
                
                OrderDetails details = orderTracking.get(String.valueOf(myReqId));
                if (details != null) {
                    details.status = "INSTRUMENT_NOT_FOUND";
                    details.errorMsg = resultString;
                }
                
                LOGGER.info("╚════════════════════════════════════════╝\n");
                return; // 返回，让系统尝试下一个instrument
            }
    ...
    public void onError(MkvFunctionCallEvent event, byte errCode, String errStr) {
        ...
        // 添加详细的错误分析
        if (errCode == -105) {
            LOGGER.error("║ *** -105 INSTRUMENT NOT FOUND ***      ║");
            LOGGER.error("║ 来源: onError回调                      ║");
        }

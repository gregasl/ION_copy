  ...
  private void initializeTraderMap() {
    ...
    private void initializeOrderExpirationChecker() {
        ...
        orderExpirationScheduler.scheduleAtFixedRate(() -> {
            try {
                LOGGER.debug("Checking for duplicate bids and offers and expired orders");
                monitorActiveOrders();
...
private void cancelOldestOrders(Map<String, Map<String, List<String>>> multipleOrders) {
    ...
    public void checkForExpiredOrders() {
        ...
        Collection<MarketOrder> orders = orderRepository.getAllOrders();
        int expiredCount = 0;
        ...
        for (MarketOrder order : orders) {
            ...
            if (order.isExpired()) {
                ...
                if (!marketSource.equals("FENICS_USREPO") || orderId == null || traderId == null) {
                    // Skip FENICS venues for and if it doesn't return an orderId or traderId
                    continue;
                }
                ...
                if (cancelOrder == null) {
                    LOGGER.warn("Failed to issue cancel for expired order: reqId={}, orderId={}, instrId={}, side={}, marketSource={}",
                        order.getMyReqId(), orderId, order.getInstrId(), order.getVerb(), marketSource);
                    continue; // Skip this order if cancel failed
                }

                // Mark the order as dead (removes from repo, updates quote, etc)
                orderRepository.markOrderDead(order);

                // Update order status flags
                orderRepository.setOrderStatus(orderId, false);
                orderRepository.updateOrderStatus(order.getInstrId(), order.getVerb(), false);

                // Log the cancellation
                LOGGER.info("Auto-cancelled expired order: reqId={}, orderId={}, instrId={}, side={}, marketSource={}",
                    order.getMyReqId(), orderId, order.getInstrId(), order.getVerb(), marketSource);
        ...
        if (expiredCount > 0) {
            LOGGER.info("Auto-cancelled {} expired orders in this check", expiredCount);
        }

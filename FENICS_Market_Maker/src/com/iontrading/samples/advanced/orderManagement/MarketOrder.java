...
public class MarketOrder implements MkvFunctionCallListener, MkvRecordListener {
   ...
   * The order status
   */
  private int orderStatus = 0; // 0 = not active, 1 = active
   ...
   * The order manager is notified that the order is active.
   */
  public void onResult(MkvFunctionCallEvent mkvFunctionCallEvent, MkvSupply mkvSupply) {
    // Log the successful order creation
    LOGGER.info("Order created successfully: reqId={}, instrId={}, verb={}", myReqId, instrId, verb);
  }
  ...
  public void setOrderStatus(int status) {
      LOGGER.info("Setting order status: reqId={}, status={}", myReqId, status);
      orderStatus = status;
      if (status == 0) {
          // If the order is no longer active, notify the order manager
          if (orderCallback != null) {
              LOGGER.info("Order is now inactive: reqId={}, orderId={}", myReqId, orderId);
              orderCallback.orderDead(this);
          }
      }
  }
  ...
  boolean isDead() {
      // An order is considered "dead" if it is no longer active
  ...
  /**
   * Gets the filled quantity of this order.
   * 
   * @return the filled quantity
   */
  private double qtyFilled = 0.0;
  ...
  public void onFullUpdate(MkvRecord mkvRecord, MkvSupply mkvSupply,
    ...
    try {
      ...
      // Get and store the order ID from the market, if not already set
      setOrderId(mkvRecord.getValue("OrigId").toString());
      double newQtyFilled = mkvRecord.getValue("QtyFill").getReal();
      boolean wasNewFill = (newQtyFilled > qtyFilled);
      if (wasNewFill) {
        LOGGER.info("New fill for order: reqId={}, orderId={}, qtyFilled={}", myReqId, getOrderId(), newQtyFilled);
        setQtyFilled(newQtyFilled);
      } else {
        LOGGER.info("Order update: reqId={}, orderId={}, qtyFilled={}", myReqId, getOrderId(), newQtyFilled);
      }

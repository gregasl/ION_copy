package com.iontrading.samples.marketDataSubscriber;
...
public class marketDataSubscriber  implements MkvPlatformListener {
    ...
    private final String REDIS_CHANNEL;
    private final String REDIS_CHANNEL_INSTRUMENT;
    private final String REDIS_CHANNEL_TRADE;
    private final String REDIS_CHANNEL_ORDER;
    private final String REDIS_CHANNEL_REF_PRICE;
    ...
    // Constructor
    public marketDataSubscriber(String[] args) {
        try {

            if (isProduction) {
                REDIS_CHANNEL = "ION:ION_MARKET_DATA";
                REDIS_CHANNEL_INSTRUMENT = "ION:ION_INSTRUMENT_DATA";
                REDIS_CHANNEL_TRADE = "ION:ION_TRADE_DATA";
                REDIS_CHANNEL_ORDER = "ION:ION_ORDER_DATA";
                REDIS_CHANNEL_REF_PRICE = "ION:ION_REF_PRICE_DATA";
                HEARTBEAT_CHANNEL = "HEARTBEAT:ION:MARKET_DATA";
                REDIS_HOST = "cacheprod";
            } else {
                REDIS_CHANNEL = "ION:ION_MARKET_DATA_UAT";
                REDIS_CHANNEL_INSTRUMENT = "ION:ION_INSTRUMENT_DATA_UAT";
                REDIS_CHANNEL_TRADE = "ION:ION_TRADE_DATA_UAT";
                REDIS_CHANNEL_ORDER = "ION:ION_ORDER_DATA_UAT";
                REDIS_CHANNEL_REF_PRICE = "ION:ION_REF_PRICE_DATA_UAT";
                REDIS_HOST = "cacheuat";
                HEARTBEAT_CHANNEL = "HEARTBEAT:ION:MARKET_DATA_UAT";
            }
    ...
    public void publishToRedis(String key, String recordName, Map<String, Object> data) {
        ...
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            logger.error("Redis connection lost, attempting to reconnect: " + jce.getMessage());
            try {
                initializeRedisConnection();
            } catch (Exception e) {
            ...
            logger.error("Error publishing to Redis: " + e.getMessage());
        }
    }
    ...
    private class DataListener implements MkvRecordListener {
        ...
        private void updateRecord(MkvRecord mkvRecord, MkvSupply mkvSupply) {
            try {
                ...
                while (cursor != -1) {
                    ...
                    if ((fieldName.startsWith("Bid") || fieldName.startsWith("Ask")) && fieldName.endsWith("Status")) {
                        try {
                            ...
                            logger.error("Error parsing attribute index from field: " + fieldName + " - " + e.getMessage());
                        } catch (Exception e) {
                            logger.error("Error processing attribute field: " + fieldName + " - " + e.getMessage());
                        }
                    }
                    // Regular way updates
                ...
                logger.error("Error processing market data update: " + e.getMessage());
            }
        }
    }
    ...
    private class InstrumentListener implements MkvRecordListener {
        ...
        private void updateRecord(MkvRecord mkvRecord, MkvSupply mkvSupply) {
            try {
                ...
                logger.error("Error processing instrument data update: " + e.getMessage());
            }
        }
    }
    ...
    private class TradeDataListener implements MkvRecordListener {
        ...
        private void updateRecord(MkvRecord mkvRecord, MkvSupply mkvSupply) {
            try {
                ...
                logger.error("Error processing trade data update: " + e.getMessage());
            }
        }
    }
            ...
            if (pattern.getName().startsWith("USD.CM_DEPTH")) {
                if (!isDepthSubscribed) {
                    try {
                        ...
                        logger.error("Error subscribing to market depth data: " + e.getMessage());
                ...
                if (!isTradeSubscribed) {
                    try {
                        ...
                        logger.error("Error subscribing to trade data: " + e.getMessage());
                ...
                if (!isInstrumentSubscribed) {
                    try {
                        ...
                        logger.error("Error subscribing to instrument data: " + e.getMessage());
                ...
                if (!isOrderSubscribed) {
                    try {
                        ...
                        logger.error("Error subscribing to order data: " + e.getMessage());
                ...
                if (!isReferencePriceSubscribed) {
                    try {
                        ...
                        logger.error("Error subscribing to reference price data: " + e.getMessage());
        ...
        try {
            ...
            if (isRedisConnected.get()) {
                try (Jedis jedis = jedisPool.getResource()) {
                    ...
                    logger.error("Failed to send final heartbeat: " + e.getMessage());
                    return false;
                }
            }
            ...
            try {
                ...
                logger.error("Executor service shutdown interrupted");
                return false;
            }
    ...
    public void waitForTermination() {
        try {
            ...
            logger.warn("Termination wait interrupted " + e);
            Thread.currentThread().interrupt();
        }
    }
    ...
    public static void main(String[] args) {
        marketDataSubscriber subscriber = null;
        try {
            subscriber = new marketDataSubscriber(args);
            if (subscriber.logger == null) {
                subscriber.logger = new IONLogger(subscriber.getMkvLog(), subscriber.getLogLevel(), "marketDataSubscriber");
           }
            subscriber.logger.info("Market Data Subscriber started successfully");
            subscriber.logger.info("Environment: " + (subscriber.isProduction ? "PRODUCTION" : "UAT"));
            subscriber.logger.info("Redis Host: " + subscriber.REDIS_HOST);
    ...
    public void onMain(MkvPlatformEvent event) {
        if (event.equals(MkvPlatformEvent.START)) {
            ...
            logger.info("PublishListener registered successfully");
            ...
            if (isReady) {
                ...
                logger.error("Error during shutdown signaling: " + e.getMessage());
            }
        }
    }
    ...
    private void sendHeartbeat() {
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            try {
                ...
                logger.error("Failed to reconnect to Redis during heartbeat: " + reconnectEx.getMessage());
            }
        }
    }
